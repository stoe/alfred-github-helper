...
const teamIcon = {
  type: 'png',
  path: path.resolve(__dirname, '../assets/team.png')
}
...
const memberIcon = {
  type: 'png',
  path: path.resolve(__dirname, '../assets/member.png')
}
...
async function* getMembers(octokit, org, team, data = []) {
  ...
  if (members.pageInfo && members.pageInfo.hasNextPage) {
    const {organization: om} = await octokit.graphql(membersQuery, {org, team, page: members.pageInfo.endCursor})

    await getMembers(octokit, org, om.team, data).next()
  }
  ...
  if (members && members.nodes) {
    for (const m of members.nodes) {
      data.push({
        ...
        type: 'default',
        valid: true,
        quicklookurl: `${m.avatarUrl}&size=420`
      })
    }
  }
...
;(async () => {
  ...
  if (Date.now() < lastcheck + 1000 * 60 * 60 * 24) {
    ...
    if (alfy.debug) {
      const debugIcon = {
        type: 'png',
        path: path.resolve(__dirname, '../assets/debug.png')
      }
    ...
    await getTeams(o.team, teams).next()
    await getMembers(octokit, org, om.team, members).next()
